...
pub struct LRU<K: ObjectId> {
    capacity: usize,
    store: LinkedHashMap<K, (), ahash::RandomState>,
}

impl<K: ObjectId> LRU<K> {
...
impl<K: ObjectId> Cache<K> for LRU<K> {
    fn write(&mut self, key: K, _timestamp: Timestamp) {
        if self.store.contains_key(&key) {
            self.store.get_refresh(&key);
        } else {
    ...
    fn report_access(&mut self, _key: K, _timestamp: Timestamp) {
        // no nothing
    }
}
